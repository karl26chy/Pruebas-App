...
export default async function attendanceSuite(world) {
  ...
  const mkPeriod = (nombre, numero, activo) => post('/academic_periods', {
    institucion_id: instC.id, nombre, numero, anio: 2026,
  ...
  await test('crear asistencia sin periodo abierto devuelve 409', async () => {
    ...
    await put(`/academic_periods/${p1.id}`, {
      institucion_id: instC.id, nombre: 'Primer periodo', numero: 1, anio: 2026,
