...
export default async function crudSuite(world) {
  ...
  await test('POST genera un id cuando no se envía', async () => {
    const res = await post('/grades', {
      institucion_id: world.inst.A.id, nombre: `Auto-${world.id}`, tipo_grado: 'C', cupo_maximo: 20,
    }, world.tokens.adminA);
  ...
  await test('POST respeta un id enviado explícitamente', async () => {
    ...
    const res = await post('/grades', {
      id: customId, institucion_id: world.inst.A.id, nombre: `Fijo-${world.id}`, tipo_grado: 'D', cupo_maximo: 15,
    }, world.tokens.adminA);
    equal(res.status, 201, 'status');
  ...
  await test('las columnas no declaradas se ignoran', async () => {
    const res = await post('/grades', {
      ...
      cupo_maximo: 10, columna_inventada: 'valor', drop_table: 'x',
    }, world.tokens.adminA);
    equal(res.status, 201, 'status');
  ...
  await test('ciclo completo: crear, leer, actualizar (PUT y PATCH) y borrar', async () => {
    const creado = (await post('/grades', {
      institucion_id: world.inst.A.id, nombre: `Ciclo-${world.id}`, tipo_grado: 'F', cupo_maximo: 12,
    ...
    const actualizado = await put(`/grades/${creado.id}`, {
      institucion_id: world.inst.A.id, nombre: 'Renombrado', tipo_grado: 'F', cupo_maximo: 18,
    }, world.tokens.adminA);
    equal(actualizado.status, 200, 'PUT status');
    equal(actualizado.data.nombre, 'Renombrado', 'nombre actualizado');
    equal(actualizado.data.cupo_maximo, 18, 'cupo actualizado');

    const parcheado = await patch(`/grades/${creado.id}`, { cupo_maximo: 22 }, world.tokens.adminA);
    equal(parcheado.status, 200, 'PATCH status');
    equal(parcheado.data.cupo_maximo, 22, 'cupo parcheado');
    equal(parcheado.data.nombre, 'Renombrado', 'PATCH no pisa otros campos');
  ...
  await test('la contraseña se guarda hasheada y permite iniciar sesión', async () => {
    ...
    const creado = (await post('/users', {
      ...
      institucion_id: world.inst.A.id, activo: true,
    }, world.tokens.adminA)).data;
