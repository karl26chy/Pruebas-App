...
/** Institución por subdominio (para el login por identificación). */
export async function institutionBySubdomain(subdominio) {
  const { rows } = await pool.query(
    'SELECT * FROM institutions WHERE LOWER(subdominio) = LOWER($1) LIMIT 1',
    [subdominio]
  );
  return rows[0] || null;
}
