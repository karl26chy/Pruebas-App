...
export function renderLiceoBoletin(data, config, ctx = {}) {
  ...
  if (hasEvalCols) {
    if (maxEvals > 0) {
      h1.push(`\\multicolumn{${N}}{>{\\columncolor{Ink}}c}{\\scriptsize \\color{white}\\bfseries EVALUACIONES}`);
    } else {
...
\\end{tabularx}
\\vspace{0.05cm}
\\colorbox{LightGold}{\\parbox{\\dimexpr\\linewidth-2\\fboxsep\\relax}{\\centering\\scriptsize Tasa de asistencia\\quad\\headingfont\\bfseries ${nint(attendance.tasa)}\\%}}`;
...
\\fancyfoot[C]{{\\setlength{\\fboxsep}{0pt}\\colorbox{Ink}{\\parbox[b][0.5cm][c]{\\linewidth}{%
