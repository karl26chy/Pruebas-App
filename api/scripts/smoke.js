...
if (!SUPER_EMAIL || !SUPER_PASSWORD) {
  console.error('Define SUPER_EMAIL y SUPER_PASSWORD para ejecutar el smoke test.');
  process.exit(1);
}
...
async function run() {
  ...
  const badLogin = await post('/auth/login', { email: SUPER_EMAIL, password: 'incorrecta' });
