...
export const DocentesTab: React.FC = () => {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showMsg('success', 'Docente creado.');
      await refreshData();
    } catch (err) {
      showMsg('error', err instanceof Error ? err.message : 'Error al crear docente.');
    }
  };
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setEditing(null);
      showMsg('success', 'Docente actualizado.');
      await refreshData();
    } catch (err) {
      showMsg('error', err instanceof Error ? err.message : 'Error al actualizar docente.');
    }
  };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fade-in">
      ...
      <Card className="lg:col-span-2">
        ...
        {feedback && (
          <div className={`mb-4 p-3 rounded-xl border text-sm ${feedback.type === 'success' ? 'bg-emerald-50 border-emerald-200 text-emerald-600' : 'bg-red-50 border-red-200 text-red-600'}`}>
            {feedback.text}
          </div>
        )}
