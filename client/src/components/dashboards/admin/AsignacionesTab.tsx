...
export const AsignacionesTab: React.FC = () => {
  ...
  const handleCreateAssignment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showMsg('error', err instanceof Error ? err.message : 'Error al crear asignación.');
    }
  };
  ...
  const handleAssignStudent = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showMsg('error', err instanceof Error ? err.message : 'Error al matricular.');
    }
  };
              ...
              misAsignaciones.map(a => (
                <div key={a.id} className="p-3 bg-white rounded-xl border border-gray-200 text-xs">
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-semibold text-gray-900">{teacherName(a.profesor_id)}</span>
            ...
            {misMatriculas.length === 0 ? (
              ...
              misMatriculas.map(sg => (
                <div key={sg.id} className="flex justify-between items-center p-3 bg-white rounded-xl border border-gray-200 text-xs">
                  <span className="font-semibold text-gray-900">{studentName(sg.estudiante_id)}</span>
