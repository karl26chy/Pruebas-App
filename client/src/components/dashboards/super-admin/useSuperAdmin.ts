...
export function useSuperAdmin() {
  ...
  /** Ocupación de un grado frente a su cupo máximo. */
  const enrollmentOf = (gradeId: string, cupoMaximo?: number) => ({
    enrolled: studentGrades.filter(sg => sg.grado_id === gradeId).length,
    cupo: cupoMaximo || 30,
  });
